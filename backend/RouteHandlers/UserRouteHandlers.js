...
const SignUp = async(req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({name: name, email: email, user: user._id }, secret, { expiresIn: "1h" } )
...
const Login = async (req, res) => {
    ...
    try {
        ...
        if (isCorrect) {
            const token = jwt.sign({ name: user.name, email: email, id: user._id }, secret, { expiresIn: "1h" } )
